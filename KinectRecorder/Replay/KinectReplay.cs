...
namespace KinectRecorder.Replay
{
    ...
    {
        BinaryReader reader; Stream stream; readonly SynchronizationContext synchronizationContext;
        // Events    
        public event EventHandler<ToolBox.ReplayColorImageFrameReadyEventArgs> ColorImageFrameReady;
        public event EventHandler<ToolBox.ReplayDepthImageFrameReadyEventArgs> DepthImageFrameReady;
        public event EventHandler<ToolBox.ReplaySkeletonFrameReadyEventArgs> SkeletonFrameReady;
        // Replay   
        ...
        public bool IsFinished
        {
            get
            {
                if (colorReplay != null && !colorReplay.IsFinished) return false;
                if (depthReplay != null && !depthReplay.IsFinished) return false;
                if (skeletonReplay != null && !skeletonReplay.IsFinished) return false;
                return true;
            }
        }
        public KinectReplay(Stream stream)
        {
            ...
            if ((options & KinectRecordOptions.Color) != 0) { colorReplay = new ReplaySystem<ReplayColorImageFrame>(); } if ((options & KinectRecordOptions.Depth) != 0)
            {
            ...
            if (colorReplay != null)
            {
                colorReplay.Start(); colorReplay.FrameReady += frame => synchronizationContext.Send(state =>
                {
                    if (ColorImageFrameReady != null)
                        ColorImageFrameReady(this, new KinectRecorder.Replay.ToolBox.ReplayColorImageFrameReadyEventArgs { ColorImageFrame = frame });
                }, null);
            }
            if (depthReplay != null)
            {
                depthReplay.Start(); depthReplay.FrameReady += frame => synchronizationContext.Send(state =>
                {
                    if (DepthImageFrameReady != null)
                        DepthImageFrameReady(this, new ToolBox.ReplayDepthImageFrameReadyEventArgs { DepthImageFrame = frame });
                }, null);
            }
            if (skeletonReplay != null)
            {
                skeletonReplay.Start(); skeletonReplay.FrameReady += frame => synchronizationContext.Send(state =>
                {
                    if (SkeletonFrameReady != null)
                        SkeletonFrameReady(this, new ToolBox.ReplaySkeletonFrameReadyEventArgs { SkeletonFrame = frame });
                }, null);
            }
        }
        public void Stop()
        {
            if (colorReplay != null) { colorReplay.Stop(); }
            if (depthReplay != null) { depthReplay.Stop(); }
            if (skeletonReplay != null) { skeletonReplay.Stop(); }
            Started = false;
        }
        public void Dispose()
        {
